...
- Aktuality channel has five MDX articles: the v7.1 VPD1 investment-call aktualita (ships as the Nejnovější block on landing), plus PF 2026 and 3 Výletná backfill from v7.
- Landing page shows the newest aktualita via `<AktualitaSpotlight />` (magazine split, clickable to detail, "Další aktuality" CTA). Replaces the old `<InvestmentHero />` block.
...
**Investment call (Záměr VPD1) lives in two places after v7.1.** The homepage no longer carries a bespoke investment block — the landing spotlight (`<AktualitaSpotlight />`) picks up the VPD1 article automatically while it's the newest aktualita. The inline `#zamer-vpd1` block on `/projekty/vpd/` still carries the figures (plocha, ČPP, IRR, …) with mustard `.osa-draft` highlights. To activate for a public launch: confirm figures with Marek Semerád, strip the `.osa-draft` wrappers on the VPD page, upload `public/dokumenty/Zamer_VPD1_zakladni_souhrn.pdf`, and swap the placeholder SVG hero on the VPD1 aktualita for a real image.
...
- **InvestmentHero.astro is orphaned.** The old `src/components/InvestmentHero.astro` is no longer imported (replaced on landing by `AktualitaSpotlight.astro`; the VPD page carries its own inline `#zamer-vpd1`). Kept on disk in case it needs revival as a standalone block; safe to delete when certain it will not be needed.
...
4. Confirm VPD1 figures with Marek. The investment call lives as an aktualita now (`src/content/aktuality/vpd1-hledame-strategickeho-partnera.mdx`) plus the inline `#zamer-vpd1` block on `/projekty/vpd/`. Both still carry mustard `.osa-draft` highlights. Once Marek signs off, strip the draft wrappers on the VPD page figures and swap the placeholder SVG hero on the aktualita for a real image.
